...
//    }
	...
	// Validate the submitted information
		$query = mysql_query("SELECT * FROM login_users WHERE fname='".$fname."'");
    ...
    //		$error = '<div class="error_message">Sorry, fobID already registered to someone.</div>';
		} else if ( $count2>0 && $row['user_id'] != $id) {
			$error = '<div class="error_message">Sorry, fobID already registered to someone else.</div>';
		
		} else if(strlen($weight) > 3) {
    		$error = '<div class="error_message">Attention! You are too fat.</div>';
		}
		else if((trim($weight) == '')) {
    		$error = '<div class="error_message">You must enter an approximate weight.</div>';
		} 	
...
*/			
			...
			$sql = "UPDATE login_users SET restricted='$restrict', fname='$fname', weight='$weight', bal='$bal' WHERE user_id = '$id'";
			$query = mysql_query($sql) or die("Fatal error: ".mysql_error());
		
			echo "<h2>Updated</h2>";
			echo "<div class='success_message'>User information updated for User ID <b>$id ($fname $lname)</b>.</div>";
...
<label>FobID</label>
<input type="text" name="fname" value="<?=stripslashes($row['fname']);?>" /><br />
<label>Weight</label>
<input type="text" name="weight" value="<?=stripslashes($row['weight']);?>" style="width:40px" maxlength="3"/>lbs<br />
<label>Current Balance</label>
$<input type="text" name="bal" value="<?=stripslashes(number_format($row['bal'],2)) ?>" style="width:60px"/><br />
